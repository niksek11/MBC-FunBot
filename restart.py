...
class restart(commands.Cog):
    ...
    @app_commands.command(name="restart", description="Restarts current song.")
    ...
    async def restart(self, interaction: discord.Interaction):
        ...
        if interaction.user.voice:
            if vc:
                await vc.seek(0)
                embed = discord.Embed(title="Music", description='Song restarted! ✅', colour=config.EMBED_COLOR)
