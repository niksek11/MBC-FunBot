...
class resume(commands.Cog):
    ...
    @app_commands.command(name="resume", description="Resumes paused song.")
    ...
    async def resume(self, interaction: discord.Interaction):
        ...
        if interaction.user.voice:
            if vc:
                if not vc.pause:
                    embed = discord.Embed(title="Music", description='Already playing ❌', colour=config.EMBED_COLOR)
                    embed.set_author(icon_url=interaction.user.display_avatar.url, name=interaction.user)
                    await interaction.followup.send(embed=embed)
                else:
                    await vc.set_pause(False)
                    embed = discord.Embed(title="Music", description='Playback resumed ▶️', colour=config.EMBED_COLOR)
                    embed.set_author(icon_url=interaction.user.display_avatar.url, name=interaction.user)
                    await interaction.followup.send(embed=embed)
            else:
